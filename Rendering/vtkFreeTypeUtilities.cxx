        ...
        for (int i = 0; i < bitmap->width; ++i)
          ...
          if(*glyph_ptr == 0)
          ...
          else if(data_ptr[3] > 0)
            {
               ...
               float blendR = (t_1_m_alpha * ((data_ptr[0] - data_min)/ data_range)) + (t_alpha * tprop_r);
               float blendG = (t_1_m_alpha * ((data_ptr[1] - data_min)/ data_range)) + (t_alpha * tprop_g);
               float blendB = (t_1_m_alpha * ((data_ptr[2] - data_min)/ data_range)) + (t_alpha * tprop_b);

               // Figure out the color.
               data_ptr[0] = static_cast<unsigned char>(data_min + data_range * blendR);
               data_ptr[1] = static_cast<unsigned char>(data_min + data_range * blendG);
               data_ptr[2] = static_cast<unsigned char>(data_min + data_range * blendB);
               data_ptr[3] = static_cast<unsigned char>(data_min + data_range * (t_alpha + data_alpha * t_1_m_alpha));
               data_ptr += 4;
               ++glyph_ptr;
            }
          else
